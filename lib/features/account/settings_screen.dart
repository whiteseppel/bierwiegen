  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            AccountTopBar(title: 'Einstellungen', scrolled: _scrolled),
            Expanded(
...
class _StepsSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        const SectionLabel('So wird gespielt'),
        const SizedBox(height: 12),
...
class _AppSection extends StatelessWidget {
                ...
                children: [
                  ...
                  _AppRow(
                    ...
                    onTap:
                        () => showLicensePage(
                          context: context,
                          applicationName: 'Bierwiegen',
                        ),
                  ),
                  const _AppRow(label: 'Version', value: _VersionText()),
                ],
              ),
            ),
          ),
  ...
  void _openLegal(BuildContext context, String title, String body) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => LegalDetailScreen(title: title, body: body),
      ),
    );
  }
  ...
  Future<void> _sendFeedback(BuildContext context) async {
    final uri = Uri(
      scheme: 'mailto',
      path: 'info@bierwiegen.info',
      query: 'subject=Feedback zu Bierwiegen',
    );
    ...
    if (!launched && context.mounted) {
      ScaffoldMessenger.of(context)
        ...
        ..showSnackBar(
          const SnackBar(
            content: Text('Kein E-Mail-Programm gefunden.'),
          ),
        );
    }
  }
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          border: Border(
            top: BorderSide(
              color: first ? Colors.transparent : const Color(0x0F000000),
            ),
          ),
        ),
        child: Row(
          children: [
            ...
            if (value != null) ...[value!, const SizedBox(width: 12)],
            if (onTap != null)
              const Text(
                '›',
                style: TextStyle(
